...
- 이상의 의존성은 애플리케이션 개발 프로레스를 간략화해주는 의존성으로, 가장 중요한 것 중 하나는 클래스 경로의 파일이 수정될 때마다 스프링부트 애플리케이션을 자동으로 재시작해준다는 것이다.
...
3. @Column : 컬럼며을 사용자화 / 컬럼의 길이 / 컬럼의 nullable 여부를 지정하는 것도 가능하다. 예시는 이하와 같다.
...
```
근데 지금 보면 어차피 @Query가 있으나 없으나 List부분응ㄴ 똑같이 작성되는 것을 알 수 있다. 그런데 어떨 때 쓰냐면 엄청 복잡한 쿼리를 작성해야할 때이거나, 아니면 Like와 같은 고급 표현식을 사용할 때 적용할 수 있다.
우리가 처음 작성한 방식대로라면
```java
System.out.println(repository.findByBrand("Kia"));
```
